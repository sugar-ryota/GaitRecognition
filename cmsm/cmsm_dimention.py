...
gallery_list = []
num = 0
df = pd.read_csv("./cnnfeature/silhouette/cnnmodel/8km/tr.csv", header=None)
df_array = np.array(df)
print(df_array.shape)
add = int(df_array.shape[1]/sub_num)
print(add)
for i in range(sub_num):
    df = pd.read_csv("./cnnfeature/silhouette/cnnmodel/8km/tr.csv",
...
for p_num in range(1, 4):  # probeデータは3つに分けているから一つずつpredictして平均をとる
    ...
    num = 0
    df = pd.read_csv(
        f"./cnnfeature/silhouette/cnnmodel/8km/ts6km_{p_num}.csv", header=None)
    df_array = np.array(df)
    print(df_array.shape)
    add = int(df_array.shape[1]/sub_num)
    print(add)
    for i in range(sub_num):
        df = pd.read_csv(
            f"./cnnfeature/silhouette/cnnmodel/8km/ts6km_{p_num}.csv", header=None, usecols=[x for x in range(num, num+add)])
