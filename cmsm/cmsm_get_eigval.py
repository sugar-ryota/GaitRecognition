...
from base_class_get_eigval import ConstrainedSMBase, MSMInterface
...
for p_num in range(1,4): #probeデータは3つに分けているから一つずつpredictして平均をとる
    ...
    probe_list = []
    num = 0
    df = pd.read_csv(f"./cnnfeature/silhouette/cnnmodel/3km/ts3km_{p_num}.csv",header=None)
    df_array = np.array(df)
    add = int(df_array.shape[1]/sub_num)
    for i in range(sub_num):
        df = pd.read_csv(f"./cnnfeature/silhouette/cnnmodel/3km/ts3km_{p_num}.csv",header=None,usecols=[x for x in range(num,num+add)])
        probe_list.append(df)
        num += add
    probe_array = np.array(probe_list)
    probe_trans = probe_array.transpose(0,2,1)
    model = ConstrainedMSM(n_subdims=5,n_gds_dims=140)
