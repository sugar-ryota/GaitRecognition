...
for i in range(sub_num):
  array = []
  total_ga_feature.append(array)
for g_num in range(1,4):
  gallery_list = []
  num = 0
  df = pd.read_csv(f"./cnnfeature/silhouette/cnnmodel/2km/tr_{g_num}.csv", header=None)
  df_array = np.array(df)
  add = int(df_array.shape[1]/sub_num)
  for i in range(sub_num):
    df = pd.read_csv(f"./cnnfeature/silhouette/cnnmodel/2km/tr_{g_num}.csv",header=None, usecols=[x for x in range(num, num+add)])
  ...
  for i in range(sub_num):
    total_ga_feature[i].extend(ga_TE_feature[i].T)

# ga_TE_feature = np.array(ga_TE_feature)
# ga_TE_feature_trans = ga_TE_feature.transpose(0, 2, 1)
total_ga_feature = np.array(total_ga_feature)
# total_ga_feature_trans = total_ga_feature.transpose(0,2,1)
#%%
...
for p_num in range(1, 4):
  ...
  num = 0
  df = pd.read_csv(
      f"./cnnfeature/silhouette/cnnmodel/2km/ts2km_1_{p_num}.csv", header=None)
  df_array = np.array(df)
  add = int(df_array.shape[1]/sub_num)
  for i in range(sub_num):
    df = pd.read_csv(
        f"./cnnfeature/silhouette/cnnmodel/2km/ts2km_1_{p_num}.csv", header=None, usecols=[x for x in range(num, num+add)])
