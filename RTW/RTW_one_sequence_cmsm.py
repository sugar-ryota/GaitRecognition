...
for g_num in range(1,4):
    ...
    for p_num in range(1,4):
        ...
        probe_list = []
        num = 0
        df = pd.read_csv(f"./cnnfeature/silhouette/cnnmodel/2km/ts2km_{g_num}_{p_num}.csv", header=None)
        df_array = np.array(df)
        add = int(df_array.shape[1]/sub_num)
        for i in range(sub_num):
            df = pd.read_csv(f"./cnnfeature/silhouette/cnnmodel/2km/ts2km_{g_num}_{p_num}.csv", header=None, usecols=[x for x in range(num, num+add)])
        ...
        print(f"g_num:{g_num} and p_num:{p_num} accuracy:{accuracy}")
        sum_acc += accuracy
        acc = sum_acc/p_num
        print(f"g_num:{g_num} total_accuracy:{acc}")
