...
from base.base_log import subspace_bases, mean_square_singular_values
...
for p_num in range(1, 4):
    ...
    num = 0
    df = pd.read_csv(
        f"./cnnfeature/silhouette/cnnmodel/7km/ts5km_{p_num}.csv", header=None)
    df_array = np.array(df)
    add = int(df_array.shape[1]/sub_num)
    for i in range(sub_num):
        df = pd.read_csv(
            f"./cnnfeature/silhouette/cnnmodel/7km/ts5km_{p_num}.csv", header=None, usecols=[x for x in range(num, num+add)])
