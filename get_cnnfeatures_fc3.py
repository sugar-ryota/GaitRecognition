...
model.save('./path/to/model_fc3.h5')
...
#     exmodel = copy.deepcopy(model)
    exmodel = load_model('path/to/model_fc3.h5')
    exmodel.fit(gxdata[trkm],gydata[trkm],epochs=epoch,batch_size=10,verbose=2) #verboseはログの出力の指定(2ならepochごとに1行のログを出力)
    outmodel = Model(inputs=exmodel.input,outputs=exmodel.layers[len(exmodel.layers)-3].output) #fc層からのCNN特徴取り出しのモデル
    ...
    wb = excel.Workbook()
    wb.save(f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr_fc3.csv') #excelファイル作成
    np.savetxt(f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr_fc3.csv',feature.T,delimiter=',') #作成したexcelファイルにgallery特徴量保存 .Tで転置
    open(f'./cnnfeature/silhouette/cnnmodel/tr_fc3{trkm}.json',"w").write(exmodel.to_json())
    exmodel.save_weights(f'./cnnfeature/silhouette/cnnmodel/tr_fc3{trkm}.h5') #重み保存
    for n in range(3):
        for tsi,tskm in enumerate(pxdata[n].keys()):
            feature = outmodel.predict(pxdata[n][tskm],batch_size=10,verbose=0)
            wb = excel.Workbook()
            wb.save(f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts_fc3{tskm}_{n+1}.csv')
            np.savetxt(f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts_fc3{tskm}_{n+1}.csv',feature.T,delimiter=',') #probe特徴量保存 3つに分けたうちの1つずつ
