...
from base.base_log import subspace_bases, mean_square_singular_values, get_log_sum
...
class SMBase(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):  # 識別で呼ばれる
        ...
        #     salt, 'some probabilities are smaller than 0! min value is {}'.format(
        #         proba.min())
        # assert proba.max() < 1.0 + \
        #     salt, 'some probabilities are bigger than 1! max value is {}'.format(
        #         proba.max())
        # proba = np.clip(proba, 0, 1)  # 0と1の範囲に納める
...
class ConstrainedSMBase(SMBase):
    ...
    def _fit(self, X, y):  # こっちが呼ばれる
        ...
        """
        # print("cmsm")
        dic = [subspace_bases(_X, self.n_subdims)
               for _X in X]  # 学習データに対して部分空間が生成される
        # dic,  (n_classes, n_dims, n_subdims)
        dic = np.array(dic)
        # all_bases, (n_dims, n_classes * n_subdims)
        all_bases = np.hstack(dic)  # 全ての部分空間を連結する
...
class MSMInterface(object):
    ...
    def predict_proba(self, X):  # こっちが呼ばれる
        """
        Predict class probabilities
        Parameters:
        -----------
        ...
        for _X in X:
            ...
            c = [get_log_sum(g, i) for g in gramians]
            i += 1
            pred.append(c)
