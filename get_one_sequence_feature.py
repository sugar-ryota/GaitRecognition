...
for i in range(3):
    gxdata.append(copy.deepcopy(dict))
    gydata.append(copy.deepcopy(dict))
    ...
    for kmi, km in enumerate(galist, 2):  # 2kmからだからindexの開始数値は2
        ...
        for pici, pic in enumerate(piclist):
            ...
            if(pici < len(piclist) / 3):
                ...
                i = 2
            gxdata[i][key].append(tmp)
            gydata[i][key].append(sbi)  # label
...
for i in range(3):
    for km in gxdata[i].keys():  # ["2km","3km",....]
        gxdata[i][km] = np.array(gxdata[i][km])
        gxdata[i][km].astype('float32')
        gydata[i][km] = np.array(gydata[i][km])
        gydata[i][km] = to_categorical(gydata[i][km])
...
for m in range(3):
  ...
  for tri, trkm in enumerate(gxdata[m].keys()):
      #     exmodel = copy.deepcopy(model)
      exmodel = load_model('path/to/model.h5')
      # verboseはログの出力の指定(2ならepochごとに1行のログを出力)
      exmodel.fit(gxdata[m][trkm], gydata[m][trkm],epochs=epoch, batch_size=10, verbose=2)
      outmodel = Model(inputs=exmodel.input, outputs=exmodel.layers[len(exmodel.layers)-2].output)  # fc層からのCNN特徴取り出しのモデル
      feature = outmodel.predict(gxdata[m][trkm], batch_size=10, verbose=0)

      if not os.path.exists('./cnnfeature/silhouette/cnnmodel/'+trkm):os.mkdir('./cnnfeature/silhouette/cnnmodel/'+trkm)  # 保存先のディレクトリ作成
      wb = excel.Workbook()
      wb.save(f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr_{m+1}.csv')  # excelファイル作成
      # 作成したexcelファイルにgallery特徴量保存 .Tで転置
      np.savetxt(f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr_{m+1}.csv', feature.T, delimiter=',')
      open(f'./cnnfeature/silhouette/cnnmodel/tr{trkm}_{m+1}.json', "w").write(exmodel.to_json())
      exmodel.save_weights(f'./cnnfeature/silhouette/cnnmodel/tr{trkm}_{m+1}.h5')  # 重み保存
      for n in range(3):
          for tsi, tskm in enumerate(pxdata[n].keys()):
              feature = outmodel.predict(pxdata[n][tskm], batch_size=10, verbose=0)
              wb = excel.Workbook()
              wb.save(f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts{tskm}_{m+1}_{n+1}.csv')
              # probe特徴量保存 3つに分けたうちの1つずつ
              np.savetxt(f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts{tskm}_{m+1}_{n+1}.csv', feature.T, delimiter=',')
              sxres = smax(exmodel, pxdata[n][tskm], plabel[n][tskm], sub_num)
              result[tri][tsi] += sxres
          print(result[tri])
  result /= (n+1)  # 3つの平均を取るために3で割る
  wb = excel.Workbook()
  wb.save(f'./results/silhouette/softmax_{m+1}.csv')
  np.savetxt(f'./results/silhouette/softmax_{m+1}.csv', result, delimiter=',')
