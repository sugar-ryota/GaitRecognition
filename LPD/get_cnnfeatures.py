...
np.savetxt(f'./cnnfeature/silhouette/cnnmodel/200.csv',
           feature.T, delimiter=',')
...
feature = outmodel.predict(pxdata, batch_size=10, verbose=0)
wb = excel.Workbook()
wb.save(f'./cnnfeature/silhouette/cnnmodel/200_probe.csv')
    # probe特徴量保存 3つに分けたうちの1つずつ
np.savetxt(f'./cnnfeature/silhouette/cnnmodel/200_probe.csv', feature.T, delimiter=',')
sxres = smax(exmodel, pxdata, plabel, sub_num)
result += sxres
