...
wb = excel.Workbook()
wb.save(f'./cnnfeature/silhouette/cnnmodel/200.csv')  # excelファイル作成
# 作成したexcelファイルにgallery特徴量保存 .Tで転置
np.savetxt(f'./cnnfeature/silhouette/cnnmodel/200.csv',feature.T, delimiter=',')
open(f'./cnnfeature/silhouette/cnnmodel/200.json', "w").write(exmodel.to_json())
exmodel.save_weights(f'./cnnfeature/silhouette/cnnmodel/200.h5')  # 重み保存
for n in range(3):
    # for tsi, tskm in enumerate(pxdata[n].keys()):
    feature = outmodel.predict(pxdata[n], batch_size=10, verbose=0)
    wb = excel.Workbook()
    wb.save(f'./cnnfeature/silhouette/cnnmodel/200_{n+1}.csv')
    # probe特徴量保存 3つに分けたうちの1つずつ
    np.savetxt(
        f'./cnnfeature/silhouette/cnnmodel/200_{n+1}.csv', feature.T, delimiter=',')
