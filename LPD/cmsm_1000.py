...
num = 0
for i in range(sub_num):
    df = pd.read_csv("./cnnfeature/silhouette/cnnmodel/1000.csv",
...
for p_num in range(1, 4):  # probeデータは3つに分けているから一つずつpredictして平均をとる
    ...
    for i in range(sub_num):
        df = pd.read_csv(
            f"./cnnfeature/silhouette/cnnmodel/1000_{p_num}.csv", header=None, usecols=[x for x in range(num, num+pr_difference[p_num-1][i])])
