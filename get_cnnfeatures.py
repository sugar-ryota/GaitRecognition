...
#     exmodel = copy.deepcopy(model)
    ...
    np.savetxt(f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr.csv',feature.T,delimiter=',') #作成したexcelファイルにgallery特徴量保存 .Tで転置
    open(f'./cnnfeature/silhouette/cnnmodel/tr{trkm}.json',"w").write(exmodel.to_json())
    exmodel.save_weights(f'./cnnfeature/silhouette/cnnmodel/tr{trkm}.h5') #重み保存
    for n in range(3):
        for tsi,tskm in enumerate(pxdata[n].keys()):
            feature = outmodel.predict(pxdata[n][tskm],batch_size=10,verbose=0)
            wb = excel.Workbook()
            wb.save(f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts{tskm}_{n+1}.csv')
            np.savetxt(f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts{tskm}_{n+1}.csv',feature.T,delimiter=',') #probe特徴量保存 3つに分けたうちの1つずつ
