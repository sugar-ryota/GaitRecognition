...
#先にランダムサンプリングを行いTEfeatureを作成してからcnnで特徴量を抽出する
...
for sbi, sb in enumerate(sblist):  # index object
    ...
    for kmi, km in enumerate(galist, 2):  # 2kmからだからindexの開始数値は2
        ...
        # piclist = ["00000001.png","00000002.png"....]
        piclist = natsorted(glob.glob(km+"/*.png"))
        rand = [random.randint(0, len(piclist)) for k in range(sample_num)]
        rand.sort()
        for pic in piclist:
...
model.save('./path/to/model_te_cnnfeature.h5')
...
for tri, trkm in enumerate(gxdata.keys()):
    ...
    exmodel = load_model('path/to/model_te_cnnfeature.h5')
    ...
    wb = excel.Workbook()
    wb.save(f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr_te_cnn.csv')  # excelファイル作成
    # 作成したexcelファイルにgallery特徴量保存 .Tで転置
    np.savetxt(
        f'./cnnfeature/silhouette/cnnmodel/{trkm}/tr_te_cnn.csv', feature.T, delimiter=',')
    open(
        f'./cnnfeature/silhouette/cnnmodel/tr{trkm}_te_cnn.json', "w").write(exmodel.to_json())
    exmodel.save_weights(
        f'./cnnfeature/silhouette/cnnmodel/tr{trkm}_te_cnn.h5')  # 重み保存
    del feature
    gc.collect()
    for tsi, tskm in enumerate(pxdata.keys()):
        ...
        wb = excel.Workbook()
        wb.save(
            f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts{tskm}_te.csv')
        np.savetxt(
            f'./cnnfeature/silhouette/cnnmodel/{trkm}/ts{tskm}_te.csv', feature.T, delimiter=',')
        sxres = smax(exmodel, pxdata[tskm], plabel[tskm], sub_num)
        result[tri][tsi] += sxres
    print(result[tri])
wb = excel.Workbook()
wb.save(f'./results/silhouette/softmax_te_cnn.csv')
np.savetxt(f'./results/silhouette/softmax_te_cnn.csv', result, delimiter=',')
